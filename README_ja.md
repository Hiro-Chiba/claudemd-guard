...
Claude Code を再起動すれば有効になります。アンインストールは `claudemd-guard uninstall` を実行してください。
...
claudemd-guard は Anthropic のエンドポイントとのみ通信します（`CLAUDEMD_GUARD_API_KEY` 設定時は Anthropic API 直接、それ以外は Claude CLI サブプロセス経由）。その他の外部サービスとの通信は行わず、テレメトリも送信しません。
